...
func (c *Client) Token(ctx context.Context, cfg *config.MountConfig) (*oauth2.Token, error) {
	...
	if cfg.PodInfo.ServiceAccountTokens != "" {
		SAToken, err := c.GetTokenFromDriver(cfg) // calling function to extract token received from driver
		if err != nil {
			return nil, fmt.Errorf("unable to fetch SA token from driver: %w", err)
		}
		SATokenVal = SAToken.Token
	} else {
		SAToken, err := c.GenerateProviderSAToken(ctx, cfg, idPool)
		if err != nil {
...
func (c *Client) GetTokenFromDriver(cfg *config.MountConfig) (*authenticationv1.TokenRequestStatus, error) {
...
func (c *Client) GenerateProviderSAToken(ctx context.Context, cfg *config.MountConfig, idPool string) (*authenticationv1.TokenRequestStatus, error) {
...
func tradeIDBindToken(ctx context.Context, client *http.Client, k8sToken, idPool, idProvider string) (*oauth2.Token, error) {
	body, err := json.Marshal(map[string]string{
		...
		"audience":             fmt.Sprintf("identitynamespace:%s:%s", idPool, idProvider), //	TODO: Does audience have to be changed to audience within token request?
